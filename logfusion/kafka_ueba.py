...
KAFKA_UEBA_CHECKPOINT_VERSION = 2
...
def run_kafka_ueba_pipeline(
    ...
    *,
    ...
    expected = {
        ...
        "orchestrator_config": {
            "batch_size": settings.batch_size, "watermark_lag_seconds": settings.watermark_lag_seconds,
            "peer_groups_path": str(Path(settings.peer_groups_path).resolve()) if settings.peer_groups_path else None,
            "rules_path": str(Path(settings.rules_path).resolve()) if settings.rules_path else None,
        },
    }
...
def _checkpoint_ueba(
    ...
    downstream = _run_downstream(
        states, max_event_time - config.watermark_lag_seconds * 1000 if max_event_time is not None else None,
        config.peer_groups_path, config.rules_path,
    )
